...
#Selecting the top 100 features of descriptors
...
system("grep -w -f Desc_Top_100 desc_top0.5per2 > desc_top0.5per3");
...
system("mv desc_top0.5per5 desc_top100");

#making hybrid file
system ("cut -d ',' -f 2- desc_top100 > desc_top1001");
system ("paste -d\",\" fingerprints_top10per desc_top1001 > hybrid");

# Calculating Blind prediction scores
system ("sh Rscripthybrid_R.sh");

# processing the blind prediction file
system ("cut -d\",\" -f1 hybrid > molid");
system ("sed 's/\"//g' Blind_pred_hybrid.txt > Blind_pred_hybrid1");
system ("sed 's/NT/\t\tNT/g' Blind_pred_hybrid1 > Blind_pred_hybrid2");
system ("cut -f 2- Blind_pred_hybrid2 > Blind_pred_hybrid3");
system ("paste molid Blind_pred_hybrid3 > Blind_pred_hybrid4");
system ("rm -fr Blind_pred_hybrid.txt Blind_pred_hybrid1 Blind_pred_hybrid2 Blind_pred_hybrid3 molid");
system ("mv Blind_pred_hybrid4 Blind_pred_hybrid.txt");
system ("tail -1 Blind_pred_hybrid.txt > score_file");
system ("sed 's/\t/,/g' score_file > hybrid_score");
...
system ("paste -d ',' hybrid_score sol_score1 per_score1 > score_file1");
system ("cut -f1 -d',' score_file1 > molid");
system ("cut -d ',' -f 3- score_file1 > scores");
system ("paste -d',' molid scores > score_file");
system ("cat final_header score_file > scores_hybrid.txt");
system ("mv scores_hybrid.txt prediction/");
system ("rm Blind_pred_hybrid.txt desc.txt fingerprints.log hybrid per_score score_file1 sol_score1 desc final_desc fingerprintsn hybrid_score per_score1 scores sol_top40 desc_top100 fingerprints fingerprints_top10per molid Prediction_score2 script_1.py desc_top1001 fingerprints1 Header permeability_top5 score_file sol1_score");
